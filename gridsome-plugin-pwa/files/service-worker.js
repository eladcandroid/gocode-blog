...
const appendToServiceWorker = async options => {
  const skipWaitingPath = _path.default.resolve(
    __dirname,
    '../utils/skip-wating.js'
  );
...
const createServiceWorker = async (context, config, queue, options) => {
  const serviceWorkerPath = _path.default.join(
    config.outDir,
    options.serviceWorkerPath
  );
  ...
  await (0, _workboxBuild.generateSW)({
    ...
    globIgnores: [options.serviceWorkerPath, 'assets/manifest/client.json'],
